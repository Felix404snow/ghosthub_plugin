# GhostHub Discord Plugin Installer
# Uso: irm "https://ghosthub.fun/install-plugin.ps1" | iex
...
Write-Host '       GhostHub  |  Plugin Discord' -ForegroundColor White
...
Write-Step 'Baixando plugin e logo...'
try {
    Invoke-WebRequest -Uri "$BaseUrl/plugin/inject.js" -OutFile $InjectFile -UseBasicParsing
    Invoke-WebRequest -Uri "$BaseUrl/plugin/renderer.js" -OutFile $RendererFile -UseBasicParsing
    $logoPlugin = Join-Path $InstallDir 'logo.png'
    try {
        Invoke-WebRequest -Uri "$BaseUrl/plugin/logo.png" -OutFile $logoPlugin -UseBasicParsing
    } catch {
        try { Invoke-WebRequest -Uri "$BaseUrl/logo.png" -OutFile $logoPlugin -UseBasicParsing } catch {}
    }
} catch {
    Write-Fail "Falha ao baixar o plugin: $($_.Exception.Message)"
    Write-Host ''
...
foreach ($logoPath in @('/discordghost.png', '/logo.png')) {
    try {
        Invoke-WebRequest -Uri ($BaseUrl + $logoPath) -OutFile $LogoPng -UseBasicParsing
        if ((Get-Item $LogoPng).Length -gt 1000) { $logoOk = $true; break }
    } catch {}
}
if ($logoOk) { Write-Ok 'Plugin e logo baixados' }
else { Write-Ok 'Plugin baixado'; Write-Warn 'Logo nao baixada; seguindo sem troca de icone.' }
...
if (-not $discordRoots -or $discordRoots.Count -eq 0) {
    Write-Fail 'Discord nao encontrado em %LOCALAPPDATA%.'
    Write-Host '  Instale o Discord desktop e rode o comando de novo.' -ForegroundColor DarkGray
    Write-Host ''
    Write-Host '  Pressione qualquer tecla para sair...' -ForegroundColor DarkGray
    try { $null = $Host.UI.RawUI.ReadKey('NoEcho,IncludeKeyDown') } catch { Start-Sleep 3 }
    return
...
if ($injectTargets.Count -eq 0) {
    ...
    Write-Host '  Abra o Discord uma vez, feche, e tente de novo.' -ForegroundColor DarkGray
    Write-Host ''
    Write-Host '  Pressione qualquer tecla para sair...' -ForegroundColor DarkGray
    try { $null = $Host.UI.RawUI.ReadKey('NoEcho,IncludeKeyDown') } catch { Start-Sleep 3 }
    return
...
Write-Step 'Injetando GhostHub no Discord...'
foreach ($t in $injectTargets) {
    ...
    if ($raw.Contains($MarkerStart)) {
        ...
        Write-Step "Atualizando inject em $($t.AppDir)" 'DarkGray'
    } else {
        $raw = $injectBlock.Trim() + "`r`n" + $raw
        Write-Step "Adicionando inject em $($t.AppDir)" 'DarkGray'
    }
...
if ($logoOk) {
    Write-Step 'Colocando fantasma da logo no Discord...'
    foreach ($t in $injectTargets) {
        Set-DiscordGhostIcon -appDir $t.AppDir -pngPath $LogoPng
    }
    Write-Ok 'Icone/atalhos atualizados'
}
...
Write-Host '   Pronto! GhostHub instalado no Discord.' -ForegroundColor Green
Write-Host '  ========================================' -ForegroundColor Green
Write-Host ''
Write-Host '  Quando abrir, procure o botao fantasma' -ForegroundColor White
Write-Host '  no canto inferior direito.' -ForegroundColor White
Write-Host ''
Write-Host '  Atualizar: irm "https://ghosthub.fun/update-plugin.ps1" | iex' -ForegroundColor DarkGray
